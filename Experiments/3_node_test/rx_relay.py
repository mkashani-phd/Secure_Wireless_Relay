...
class RX:
    def __init__(self, conf:config.CONFIG = config.CONFIG(), usrp:uhd.usrp.MultiUSRP = None):
        self.conf = conf
        if usrp is None:
    ...
    def record(self):
        ...
        if self.conf.MIMO:
            ...
            file = "Relay_"+str(np.round(self.usrp.get_rx_freq(),2))+"_"+str(np.round(self.usrp.get_rx_rate(),2))+"_"+str(self.conf.RX_GAIN)+"_"+str(self.conf.ACQ_TIME) + "_"+ str(self.conf.IN_CHAMBER)+"_.iq"
            f = open(file,"wb")
            start = time.time()
            cnt = 4
...
class Demodulation:
    ...
    def compute_hard_desicion_and_rs(self, signal, symbol_length, tone_bins, message_bits_for_sure=None):
        ...
        preamble_len = int(len(self.conf.PREAMBLE) * self.conf.TX_SPS * self.conf.RX_RATE / self.conf.TX_RATE)
        start_index = int(1.15 * preamble_len)
        end_index = len(signal) - int(0.985 * preamble_len)
...
class PostProcessing:
    def __init__(self,  file:str, conf:config.CONFIG = config.CONFIG(), demod:Demodulation = Demodulation()):
        ...
        plt.title("Relay signal t0", fontsize=30)
        plt.show()
...
def test():
    ...
    if(pp.check()):
        pp.push_to_db(collection = mydb['1D_SC_Relay'])
