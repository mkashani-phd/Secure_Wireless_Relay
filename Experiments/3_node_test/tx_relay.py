...
def test():
    ...
    payload_bits = np.array(string_to_bits(loaded_dict['msg_hard_decision']))
    MAC = hmac.new(conf.MAC_KEY.encode('utf-8'), msg=loaded_dict['msg_hard_decision'].encode('utf-8'), digestmod='sha256').hexdigest()
 ...
 # 
    ...
    usrp = uhd.usrp.MultiUSRP(f"serial={conf.RELAY}")  # Replace with your USRP's serial or remove parameter for default
    # Transmit signal
    for i in range(10):
        usrp.send_waveform(fsk_signal, len(fsk_signal)/conf.TX_RATE  , conf.FREQ, conf.TX_RATE, conf.CHANNEL, conf.TX_RELAY_GAIN)
        time.sleep(0.1)
